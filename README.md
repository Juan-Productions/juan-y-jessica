...
- **Velocidad de la búsqueda**: Apps Script tarda ~2 s aun en el mejor caso (y ~1 s más si lee la hoja), y a veces se cuelga; eso no se puede eliminar desde el sitio, así que `index.html` lo esconde: (1) al tocar el campo del nombre "despierta" al script; (2) mientras se escribe, adelanta la consulta (pausa de 350 ms); (3) recuerda las respuestas 2 minutos y, si ya trajo *todos* los resultados de un texto más corto (menos de 8), filtra en el momento al escribir más letras ("ver" → "vergara" sin otra consulta); (4) si un intento no responde en 3.5 s lanza otro en paralelo (hasta 3) y gana el primero, con un tope de 16 s antes de mostrar el error. Las constantes (`SEARCH_*`) están junto a `SHEET_ENDPOINT`.
