// Portada animada — sello de cera de lacre + paisaje ilustrado que se
// revela. Adaptado del proyecto de animación (Claude Design) del usuario;
// esta versión tiene su propio reloj (no depende de animations-v3.jsx / el
// panel de tweaks, que son solo herramientas del editor): el tiempo T
// arranca en 0 y no avanza hasta el primer toque, para poder reproducir
// audio (los navegadores bloquean el autoplay de sonido sin un gesto del
// usuario) y para que "TOCA PARA ABRIR" tenga sentido.
//
// Cargado vía <x-import component-from-global-scope="IntroCover" ...> en
// index.html. Se monta como overlay fijo a pantalla completa; al tocar
// "Más información" llama a window.__triggerOpenInvitation() (definida por
...
var MOTION = {
  enter: function (from, to, start, end) { return animate({ from: from, to: to, start: start, end: end, ease: Easing.easeOutCubic }); },
  draw: function (from, to, start, end) { return animate({ from: from, to: to, start: start, end: end, ease: Easing.easeInOutQuad }); },
  pop: function (from, to, start, end) { return animate({ from: from, to: to, start: start, end: end, ease: Easing.easeOutBack }); },
};
...
function Monogram(props) {
  var initials = props.initials, size = props.size, tone = props.tone;
  return (
    <div style={{ position: 'absolute', left: 0, top: 0, width: W, height: H, pointerEvents: 'none' }}>
      <div style={{
        position: 'absolute', left: 370, top: 640, transform: 'translate(-50%,-50%)',
        fontFamily: script, fontSize: size, lineHeight: 1, whiteSpace: 'nowrap', color: tone,
...
function CardPanel(props) {
  var side = props.side, rot = props.rot, op = props.op, initials = props.initials, monoSize = props.monoSize, monoTone = props.monoTone;
  var left = side === 'l';
  return (
    <div style={{
      position: 'absolute', top: 0, height: H,
      left: left ? 0 : SPLIT, width: left ? SPLIT : W - SPLIT,
      ...
      <div style={{ position: 'absolute', top: 0, left: left ? 0 : -SPLIT, width: W, height: H }}>
        <img src={SEAL_IMG} width={W} height={H} style={{ display: 'block' }} alt="" />
        <Monogram initials={initials} size={monoSize} tone={monoTone} />
      </div>
      <div style={{
        position: 'absolute', inset: 0,
        background: left
          ? 'linear-gradient(to right, rgba(64,46,28,' + (0.30 * clamp(rot / 80, 0, 1)) + ') 0%, rgba(255,252,244,0) 45%, rgba(255,252,244,' + (0.42 * clamp(rot / 80, 0, 1)) + '))'
          : 'linear-gradient(to left, rgba(64,46,28,' + (0.30 * clamp(rot / 80, 0, 1)) + ') 0%, rgba(255,252,244,0) 45%, rgba(255,252,244,' + (0.42 * clamp(rot / 80, 0, 1)) + '))',
      }} />
    </div>
  );
}
...
function Line(props) {
  return <div style={Object.assign({ fontFamily: props.family, fontSize: props.size, lineHeight: 1.28, whiteSpace: 'pre-line' }, props.style)}>{props.text}</div>;
}

function Butterfly(props) {
  var T = props.T, x = props.x, y = props.y, scale = props.scale, hue = props.hue, phase = props.phase, speed = props.speed, drift = props.drift;
  var t = T * speed + phase;
  ...
  function wing(side) {
    return {
      ...
      left: side > 0 ? '50%' : 'auto', right: side > 0 ? 'auto' : '50%',
      width: 13, height: 17,
      ...
      boxShadow: '0 1px 3px rgba(70,54,34,0.18)', border: '0.5px solid rgba(120,96,64,0.35)',
    };
  }
  function lower(side) {
    return Object.assign(wing(side), {
      top: 11, width: 10, height: 12, opacity: 0.9,
  ...
  return (
    <div style={{
      position: 'absolute', left: px, top: py, width: 0, height: 0,
      transform: 'rotate(' + tilt + 'deg) scale(' + scale + ')', transformStyle: 'preserve-3d',
    }}>
      <div style={{ position: 'absolute', left: -1, top: 4, width: 2, height: 15, borderRadius: 2, background: 'rgba(108,86,58,0.6)' }} />
      <div style={wing(1)} />
      <div style={wing(-1)} />
      <div style={lower(1)} />
      <div style={lower(-1)} />
...
var BUTTERFLIES = [
  { x: 128, y: 852, scale: 1.2, hue: 'rgba(255,253,247,0.95)', phase: 0, speed: 0.9, drift: 118 },
  { x: 545, y: 905, scale: 1.0, hue: 'rgba(252,248,236,0.92)', phase: 2.4, speed: 0.72, drift: 132 },
  { x: 320, y: 1060, scale: 1.45, hue: 'rgba(255,255,250,0.96)', phase: 4.1, speed: 1.05, drift: 96 },
  { x: 600, y: 760, scale: 0.82, hue: 'rgba(250,246,234,0.9)', phase: 1.3, speed: 0.62, drift: 104 },
  { x: 92, y: 1105, scale: 0.9, hue: 'rgba(253,250,240,0.9)', phase: 5.6, speed: 0.8, drift: 86 },
];
...
function Sparkle(props) {
  ...
  for (var i = 0; i < 70; i++) {
    ...
    dots.push(<div key={i} style={{
      position: 'absolute', left: cx + Math.cos(a) * r, top: cy + Math.sin(a) * r * 0.85 + burst * burst * 90,
      width: 5, height: 5, borderRadius: 5,
      background: '#fffdf2', opacity: clamp(o, 0, 1),
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      {rise > 0 && (
        <div style={{
          position: 'absolute', left: cx - 1, top: tailY, width: 2, height: 120 * rise,
          background: 'linear-gradient(to bottom, rgba(255,246,220,0.85), rgba(255,246,220,0))',
          opacity: rise * 0.9, filter: 'blur(1px)',
        }} />
      )}
      {rise > 0 && (
        <div style={{
          position: 'absolute', left: cx - 2, top: tailY, width: 4, height: 4, borderRadius: 4,
          background: '#fffbf0', opacity: rise,
          boxShadow: '0 0 14px 6px rgba(255,243,210,0.9)',
        }} />
      )}
      <div style={{
        position: 'absolute', left: cx - 90, top: cy - 90, width: 180, height: 180, borderRadius: 180,
...
function useIntroClock() {
  ...
  var started = startedState[0], setStarted = startedState[1];
  ...
  React.useEffect(function () {
    ...
    function step(ts) {
      ...
      // can deliver a huge gap between rAF callbacks when it resumes —
      // without this, T would jump forward by however long the tab was
      // hidden instead of advancing at a normal pace.
  ...
  return { T: T, started: started, setStarted: setStarted, revealed: revealed, setRevealed: setRevealed };
}

function IntroCover() {
  var clock = useIntroClock();
  var T = clock.T, started = clock.started, revealed = clock.revealed;
  ...
  React.useEffect(function () {
    return function () {
      if (introAudioRef.current) { try { introAudioRef.current.pause(); } catch (e) {} }
    };
  }, []);
  ...
  var blockA = clamp(Math.min(
    MOTION.enter(0, 1, CUES.Historia + 0.2, CUES.Historia + 1.4)(T),
    MOTION.enter(1, 0, CUES.Acompanas - 0.9, CUES.Acompanas - 0.1)(T)), 0, 1);
  var blockB = clamp(Math.min(
    MOTION.enter(0, 1, CUES.Acompanas + 0.2, CUES.Acompanas + 1.4)(T),
    MOTION.enter(1, 0, CUES.Destello - 0.5, CUES.Destello + 0.2)(T)), 0, 1);
    ...
    <div ref={rootRef} style={{
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: '#fff' }}>
          <div style={{ position: 'absolute', inset: 0, opacity: landOp, transform: 'scale(' + landZoom + ')', transformOrigin: '50% 46%' }}>
            <img src={LAND_IMG} width={W} height={H} style={{ display: 'block' }} alt="" />
          </div>
          <div style={{ position: 'absolute', inset: 0, opacity: landOp, transform: 'scale(' + landZoom + ')', transformOrigin: '50% 46%', pointerEvents: 'none' }}>
            <div style={{
          ...
          <div style={{ position: 'absolute', inset: 0, opacity: bflyOp, perspective: 600, pointerEvents: 'none' }}>
            {BUTTERFLIES.map(function (b, i) { return <Butterfly key={i} T={T} x={b.x} y={b.y} scale={b.scale} hue={b.hue} phase={b.phase} speed={b.speed} drift={b.drift} />; })}
          </div>
          ...
          {!cardGone && (
            <div style={{ position: 'absolute', inset: 0, perspective: 1500, perspectiveOrigin: '50% 50%' }}>
              <div style={{ position: 'absolute', inset: 0, transform: 'scale(' + cardScale + ')', transformStyle: 'preserve-3d' }}>
                <CardPanel side="l" rot={rot} op={panelOp} initials={initials} monoSize={monoSize} monoTone={monoTone} />
                <CardPanel side="r" rot={rot} op={panelOp} initials={initials} monoSize={monoSize} monoTone={monoTone} />
              </div>
              <div style={{
                position: 'absolute', left: 0, right: 0, top: 962, textAlign: 'center',
                fontFamily: serif, fontStyle: 'italic', fontWeight: 700, fontSize: 34,
                letterSpacing: '0.14em', color: '#fffdf6', opacity: tocaOp,
          ...
          <button
            ...
            style={{
              position: 'absolute', left: '50%', top: 900, transform: 'translateX(-50%)',
              opacity: moreInfoOp, pointerEvents: moreInfoOp > 0.05 ? 'auto' : 'none',
              fontFamily: serif, fontStyle: 'italic', fontWeight: 600, fontSize: 30,
              letterSpacing: '0.03em', color: '#3a2c14',
              background: 'rgba(255,253,246,0.92)', border: '1px solid rgba(120,90,58,0.55)',
              borderRadius: 999, padding: '16px 40px', cursor: 'pointer',
              boxShadow: '0 10px 30px rgba(0,0,0,0.25)', transition: 'opacity 0.3s ease',
            }}
      ...
      {!started && (
        <button onClick={handleStart} aria-label="Toca para abrir" style={{
